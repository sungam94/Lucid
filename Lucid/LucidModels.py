...
gd = importlib.import_module("guided-diffusion.guided_diffusion.script_util")
print(gd.__name__)
# from foobar import guided_diffusion
import lpips
import torch

from CLIP import clip

# from guided-diffusion.guided_diffusion.script_util import (create_model_and_diffusion,
...
def get_model_config(args):
    model_config = gd.model_and_diffusion_defaults()
    model_config.update(args.model_settings.get(args.models.model))
    # timestep_respacing = f'ddim{args.steps}'
    timestep_respacing = f"{args.steps}"
    diffusion_steps = (
...
def get_models(args, device):
    ...
    print(model_config)
    model, diffusion = gd.create_model_and_diffusion(**model_config)
    model.load_state_dict(
        torch.load(
            f"{args.env.root_path}/models/{args.models.model}.pt", map_location="cpu"
        )
    )
    model.requires_grad_(False).eval().to(device)
    if model_config["use_fp16"]:
        model.convert_to_fp16()
    if args.model_settings.get(f"{args.models.model}").use_fp16:
        model.convert_to_fp16()
    ## CLIP
    clip_model = (
        clip.load(args.models.clip_model, jit=False)[0]
        .eval()
