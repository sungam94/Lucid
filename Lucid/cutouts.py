...
class MakeCutoutsDango(nn.Module):
    def __init__(
        self, args, cut_size, Overview=4, InnerCrop=0, IC_Size_Pow=0.5, IC_Grey_P=0.2
    ):
        super().__init__()
        self.args = args
        self.cut_size = cut_size
    ...
    def forward(self, input):
        ...
        if self.Overview > 0:
            ...
            if self.args.cutout_debug:
                TF.to_pil_image(cutouts[0].clamp(0, 1).squeeze(0)).save(
                    f"{self.args.root_path}/cutout_overview0.jpg", quality=99
                )
        ...
        if self.InnerCrop > 0:
            ...
            if self.args.cutout_debug:
                TF.to_pil_image(cutouts[-1].clamp(0, 1).squeeze(0)).save(
                    f"{self.args.root_path}/cutout_InnerCrop.jpg", quality=99
                )
        cutouts = torch.cat(cutouts)
        if self.args.skip_augs is not True:
